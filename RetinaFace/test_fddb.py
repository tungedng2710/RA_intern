...
import time
#from train import show_params_gflops
...
if __name__ == '__main__':
    ...
    if args.network == "mobile0.25":
        print('Loading config mobile0.25')
        cfg = cfg_mnet
    elif args.network == "resnet50":
        print('Loading config resnet50')
        cfg = cfg_re50
    elif args.network == 'resnet18':
        print('Loading config resnet18')
        cfg = cfg_re18
    # net and model
    net = RetinaFace(cfg=cfg, phase = 'test')

    net = load_model(net, args.trained_model, args.cpu)
    #show_params_gflops(net, (3, 840, 840), False)
    net.eval()
    ...
    for i, image_path in enumerate(tqdm(images)):
        ...
        for i, bbox in enumerate(dets):
            bbox_data = {
                ...
                'pred_bbox_x2': bbox[2],
                'pred_bbox_y2': bbox[3],
                'pred_bbox_conf': bbox[4],
                'pred_lm_1': (landms[i][0], landms[i][1]),
                'pred_lm_2': (landms[i][2], landms[i][3]),
                'pred_lm_3': (landms[i][4], landms[i][5]),
                'pred_lm_4': (landms[i][6], landms[i][7]),
                'pred_lm_5': (landms[i][8], landms[i][9]),
                'class': 'human'
            }
            bbox_data_list.append(bbox_data)

        base_img_name = img_name.split('.jpg')[0]
        data_pred_dict[base_img_name] = {
            'img_h': im_height,
            'img_w': im_width,
            'prediction': bbox_data_list
        }
        # data_pred = {
        #     'img_name': img_name,
        #     'img_h': im_height,
        ...
        # with open('data_lm.txt', 'a') as file:
        #     line = f'# {image_path}'
        #     file.write(line + os.linesep)
        #     for lm in landms:
        #         lm_str = str(lm).replace('\n', '').split('[')[1].split(']')[0]
        #         lm_str = lm_str.strip().split()
        #         lm_str.insert(2, '0.0')
        #         lm_str.insert(5, '0.0')
        #         lm_str.insert(8, '0.0')
        #         lm_str.insert(11, '0.0')
        #         lm_str = ' '.join(lm_str)
        #         file.write(lm_str + os.linesep)
        ...
        #     lm_str.insert(11, '0.0')
        #     lm_str = ' '.join(lm_str)
        #     img_listt.append(image_path)
        #     lm_listt.append(lm_str)
        ...
        _t['misc'].toc()
        forward_time.append(_t['forward_pass'].average_time)
        misc_time.append(_t['misc'].average_time)
        # save dets
    ...
    print(sum(misc_time) / len(misc_time))
    fw.close()
